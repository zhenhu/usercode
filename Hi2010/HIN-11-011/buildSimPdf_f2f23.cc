...
bool buildPdf(RooWorkspace &ws, bool hi, int bkgdModel = 3, bool TrkRotBkgd = 0) {
	...
	RooRealVar x2("x2", "#chi_{2}", 0.3, -1, 1);
	...
	if (!hi){
		...
		nsig3 = new RooFormulaVar("nsig3", "@0*(@1-@2)", RooArgList(nsig1, f23, f2));
	}
	else {
		nsig2 = new RooFormulaVar("nsig2", "@0*@1*@2", RooArgList(nsig1,f2,x2)); 
    	nsig3 = new RooFormulaVar("nsig3", "@0*@3*@4-@0*@1*@2", RooArgList(nsig1, f2, x2, f23, x23));
	}
