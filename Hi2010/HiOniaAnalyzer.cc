...
private:
  ...
  bool selStandAloneMuon(const pat::Muon* aMuon);
  ...
  float Reco_QQ_ctauTrue[100];    // true ctau
   ...
   float muPlusDz[100];
   int muPlusNhits[100];
   int muPlusNPixelhits[100];
   int muPlusNPxlLayers[100];
   float muPlusInnerChi[100];
   float muPlusGlobalChi[100];
   int muPlusNMuonhits[100];
   float muPlusDB[100];

   float muMinusDxy[100];
   float muMinusDz[100];
   int muMinusNhits[100];
   int muMinusNPixelhits[100];
   int muMinusNPxlLayers[100];
   float muMinusInnerChi[100];
   float muMinusGlobalChi[100];
   int muMinusNMuonhits[100];
   float muMinusDB[100];
...
HiOniaAnalyzer::fillTreeMuon(const pat::Muon* muon, int iType, int trigBits) {
  ...
  //vMuon.SetPtEtaPhiM(muon->outerTrack()->pt(),muon->outerTrack()->eta(),muon->outerTrack()->phi(),0.1056583);
...
HiOniaAnalyzer::fillTreeJpsi(int iSign, int count) {
 ...
 //float theMass = aJpsiCand->mass();
 //if(theMass < 14.0 && theMass > 7.0){
  ...
  //vMuon1.SetPtEtaPhiM(muon1->outerTrack()->pt(),muon1->outerTrack()->eta(),muon1->outerTrack()->phi(),0.1056583);
  //vMuon2.SetPtEtaPhiM(muon2->outerTrack()->pt(),muon2->outerTrack()->eta(),muon2->outerTrack()->phi(),0.1056583);
  ...
  if (muon1->charge() > muon2->charge()) {
    ...
    new((*Reco_QQ_mumi_4mom)[Reco_QQ_size])TLorentzVector(vMuon2);
       muPlusDxy[Reco_QQ_size] = muon1->innerTrack()->dxy(RefVtx);
       muPlusDz[Reco_QQ_size] = muon1->innerTrack()->dz(RefVtx);
       muPlusNhits[Reco_QQ_size] = muon1->innerTrack()->numberOfValidHits();
       muPlusNPixelhits[Reco_QQ_size] = muon1->innerTrack()->hitPattern().numberOfValidPixelHits();
       muPlusNPxlLayers[Reco_QQ_size] = muon1->innerTrack()->hitPattern().pixelLayersWithMeasurement();
       muPlusInnerChi[Reco_QQ_size] = muon1->innerTrack()->chi2()/muon1->innerTrack()->ndof();
       muPlusGlobalChi[Reco_QQ_size] = muon1->globalTrack()->chi2()/muon1->globalTrack()->ndof();
	   //muPlusGlobalChi[Reco_QQ_size] = muon1->outerTrack()->chi2()/muon1->outerTrack()->ndof();
       muPlusNMuonhits[Reco_QQ_size] = muon1->globalTrack()->hitPattern().numberOfValidMuonHits();
	   //muPlusNMuonhits[Reco_QQ_size] = muon1->outerTrack()->numberOfValidHits();
	   muPlusDB[Reco_QQ_size] = muon1->dB();

       muMinusDxy[Reco_QQ_size] = muon2->innerTrack()->dxy(RefVtx);
       muMinusDz[Reco_QQ_size] = muon2->innerTrack()->dz(RefVtx);
       muMinusNhits[Reco_QQ_size] = muon2->innerTrack()->numberOfValidHits();
       muMinusNPixelhits[Reco_QQ_size] = muon2->innerTrack()->hitPattern().numberOfValidPixelHits();
       muMinusNPxlLayers[Reco_QQ_size] = muon2->innerTrack()->hitPattern().pixelLayersWithMeasurement();
       muMinusInnerChi[Reco_QQ_size] = muon2->innerTrack()->chi2()/muon2->innerTrack()->ndof();
       muMinusGlobalChi[Reco_QQ_size] = muon2->globalTrack()->chi2()/muon2->globalTrack()->ndof();
       //muMinusGlobalChi[Reco_QQ_size] = muon2->outerTrack()->chi2()/muon2->outerTrack()->ndof();
	   muMinusNMuonhits[Reco_QQ_size] = muon2->globalTrack()->hitPattern().numberOfValidMuonHits();
       //muMinusNMuonhits[Reco_QQ_size] = muon2->outerTrack()->numberOfValidHits();
	   muMinusDB[Reco_QQ_size] = muon2->dB();
  }
  else {
    new((*Reco_QQ_mupl_4mom)[Reco_QQ_size])TLorentzVector(vMuon2);
    new((*Reco_QQ_mumi_4mom)[Reco_QQ_size])TLorentzVector(vMuon1);
       muPlusDxy[Reco_QQ_size] = muon2->innerTrack()->dxy(RefVtx);
       muPlusDz[Reco_QQ_size] = muon2->innerTrack()->dz(RefVtx);
       muPlusNhits[Reco_QQ_size] = muon2->innerTrack()->numberOfValidHits();
       muPlusNPixelhits[Reco_QQ_size] = muon2->innerTrack()->hitPattern().numberOfValidPixelHits();
       muPlusNPxlLayers[Reco_QQ_size] = muon2->innerTrack()->hitPattern().pixelLayersWithMeasurement();
       muPlusInnerChi[Reco_QQ_size] = muon2->innerTrack()->chi2()/muon2->innerTrack()->ndof();
       muPlusGlobalChi[Reco_QQ_size] = muon2->globalTrack()->chi2()/muon2->globalTrack()->ndof();
       //muPlusGlobalChi[Reco_QQ_size] = muon2->outerTrack()->chi2()/muon2->outerTrack()->ndof();
	   muPlusNMuonhits[Reco_QQ_size] = muon2->globalTrack()->hitPattern().numberOfValidMuonHits();
       //muPlusNMuonhits[Reco_QQ_size] = muon2->outerTrack()->numberOfValidHits();
	   muPlusDB[Reco_QQ_size] = muon2->dB();

       muMinusDxy[Reco_QQ_size] = muon1->innerTrack()->dxy(RefVtx);
       muMinusDz[Reco_QQ_size] = muon1->innerTrack()->dz(RefVtx);
       muMinusNhits[Reco_QQ_size] = muon1->innerTrack()->numberOfValidHits();
       muMinusNPixelhits[Reco_QQ_size] = muon1->innerTrack()->hitPattern().numberOfValidPixelHits();
       muMinusNPxlLayers[Reco_QQ_size] = muon1->innerTrack()->hitPattern().pixelLayersWithMeasurement();
       muMinusInnerChi[Reco_QQ_size] = muon1->innerTrack()->chi2()/muon1->innerTrack()->ndof();
       muMinusGlobalChi[Reco_QQ_size] = muon1->globalTrack()->chi2()/muon1->globalTrack()->ndof();
       //muMinusGlobalChi[Reco_QQ_size] = muon1->outerTrack()->chi2()/muon1->outerTrack()->ndof();
	   muMinusNMuonhits[Reco_QQ_size] = muon1->globalTrack()->hitPattern().numberOfValidMuonHits();
       //muMinusNMuonhits[Reco_QQ_size] = muon1->outerTrack()->numberOfValidHits();
	   muMinusDB[Reco_QQ_size] = muon1->dB();
  }
  
  TLorentzVector vJpsi = lorentzMomentum(aJpsiCand->p4());
  //TLorentzVector vJpsi = vMuon1+vMuon2;
...
HiOniaAnalyzer::makeCuts(int sign) {
  ...
  if (collJpsi.isValid()) {
    ...
    //if (checkCuts(cand,muon1,muon2,&HiOniaAnalyzer::selStandAloneMuon,&HiOniaAnalyzer::selStandAloneMuon)){
...
bool 
HiOniaAnalyzer::selStandAloneMuon(const pat::Muon* aMuon) {
...
HiOniaAnalyzer::InitTree()
{
  ...
  myTree->Branch("muPlusDxy",muPlusDxy,"muPlusDxy[Reco_QQ_size]/F");
  myTree->Branch("muPlusDz",muPlusDz,"muPlusDz[Reco_QQ_size]/F");
  myTree->Branch("muPlusNhits",muPlusNhits,"muPlusNhits[Reco_QQ_size]/I");
  myTree->Branch("muPlusNPixelhits",muPlusNPixelhits,"muPlusNPixelhits[Reco_QQ_size]/I");
  myTree->Branch("muPlusNPxlLayers",muPlusNPxlLayers,"muPlusNPxlLayers[Reco_QQ_size]/I");
  myTree->Branch("muPlusInnerChi",muPlusInnerChi,"muPlusInnerChi[Reco_QQ_size]/F");
  myTree->Branch("muPlusGlobalChi",muPlusGlobalChi,"muPlusGlobalChi[Reco_QQ_size]/F");
  myTree->Branch("muPlusNMuonhits",muPlusNMuonhits,"muPlusNMuonhits[Reco_QQ_size]/I");
  myTree->Branch("muPlusDB",muPlusDB,"muPlusDB[Reco_QQ_size]/F");

  myTree->Branch("muMinusDxy",muMinusDxy,"muMinusDxy[Reco_QQ_size]/F");
  myTree->Branch("muMinusDz",muMinusDz,"muMinusDz[Reco_QQ_size]/F");
  myTree->Branch("muMinusNhits",muMinusNhits,"muMinusNhits[Reco_QQ_size]/I");
  myTree->Branch("muMinusNPixelhits",muMinusNPixelhits,"muMinusNPixelhits[Reco_QQ_size]/I");
  myTree->Branch("muMinusNPxlLayers",muMinusNPxlLayers,"muMinusNPxlLayers[Reco_QQ_size]/I");
  myTree->Branch("muMinusInnerChi",muMinusInnerChi,"muMinusInnerChi[Reco_QQ_size]/F");
  myTree->Branch("muMinusGlobalChi",muMinusGlobalChi,"muMinusGlobalChi[Reco_QQ_size]/F");
  myTree->Branch("muMinusNMuonhits",muMinusNMuonhits,"muMinusNMuonhits[Reco_QQ_size]/I");
  myTree->Branch("muMinusDB",muMinusDB,"muMinusDB[Reco_QQ_size]/F");
