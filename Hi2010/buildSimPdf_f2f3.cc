...
bool buildPdf(RooWorkspace &ws, bool hi, int bkgdModel = 3, bool TrkRotBkgd = 0) {
	...
	RooRealVar nsig1("nsig1", "N_{1S}", 100, -100, 1e6);
	//RooRealVar f23("f23", "(2S+3S)/1S", 0.1);
	//f23.setConstant(false);
	RooRealVar f3("f3", "3S/1S", 0.05);
	RooRealVar f2("f2", "2S/1S", 0.2);
	f3.setConstant(false);
	f2.setConstant(false);
    
	//RooRealVar x23("x23", "#chi_{23}", 0.1);
	//x23.setConstant(false);
	RooRealVar x2("x2", "#chi_{2}", 0.3);
	x2.setConstant(false);
    RooRealVar x3("x3", "#chi_{3}", 0.1);
