...
class MuonSelectorUpsilon : public edm::EDProducer {

    public:
        /// constructor
        MuonSelectorUpsilon(const edm::ParameterSet &iConfig) ;
        /// destructor
        ~MuonSelectorUpsilon() ;
...
MuonSelectorUpsilon::MuonSelectorUpsilon(const edm::ParameterSet &iConfig) :
...
MuonSelectorUpsilon::~MuonSelectorUpsilon() 
{
}

/// Produce: the method where we do something
void 
MuonSelectorUpsilon::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) 
{
    ...
    for (size_t i = 0, n = src->size(); i < n; ++i) {
        ...
        if (selectGlobalMuons_) {
            ...
            if (mu.isTrackerMuon() && mu.track()->numberOfValidHits() > 10 && muon::isGoodMuon(mu,muon::TMOneStationTight) && mu.track()->chi2()/mu.track()->ndof()< 5.0) {
                out->push_back(muonRef);
            }
        }
