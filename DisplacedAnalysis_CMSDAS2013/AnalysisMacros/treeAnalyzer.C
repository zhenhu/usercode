...
bool treeAnalyzer::acceptanceCuts( std::vector<TreeDipseudoLeptonCandidate>::const_iterator cand )
{
...
bool treeAnalyzer::trackSelectionCuts( std::vector<TreeDipseudoLeptonCandidate>::const_iterator cand, const bool removeIsolationCut )
{
...
bool treeAnalyzer::lifetimeRelatedCuts( std::vector<TreeDipseudoLeptonCandidate>::const_iterator cand, const bool removeLifetimeRelatedCuts, const bool decayLengthSigniInverted)
{
  // std::cout<<removeLifetimeRelatedCuts<<","<<decayLengthSigniInverted<<std::endl;
  if(decayLengthSigniInverted==1){
    if((removeLifetimeRelatedCuts || cand->leptonD0SignificanceH > d0SignificanceCut_) &&
        (removeLifetimeRelatedCuts || cand->leptonD0SignificanceL > d0SignificanceCut_) &&
...
bool treeAnalyzer::dileptonSelectionCuts( std::vector<TreeDipseudoLeptonCandidate>::const_iterator cand )
{
  // Get original leptons
  TreeLepton leptonL = candidates->leptons_[cand->leptonIndexL];
  TreeLepton leptonH = candidates->leptons_[cand->leptonIndexH];
  ...
  if((cand->leptonChargeL*cand->leptonChargeH < 0) &&
      ...
      !(leptonL.isStandAloneMuon && leptonH.isStandAloneMuon) && // we are not using standAloneMuons
      ( electrons_ || cand->corrDileptonMass > 15.)&& ( !electrons_ || cand->caloCorrMass > 15.)
  ) return true;
  return false;
}

bool treeAnalyzer::triggerMatching( std::vector<TreeDipseudoLeptonCandidate>::const_iterator cand )
{
  // Get original leptons
  TreeLepton leptonL = candidates->leptons_[cand->leptonIndexL];
  TreeLepton leptonH = candidates->leptons_[cand->leptonIndexH];

  if( leptonL.triggerMatch != 0 && leptonH.triggerMatch != 0 ) return true;
  return false;
}

bool treeAnalyzer::analysisCuts(std::vector<TreeDipseudoLeptonCandidate>::const_iterator cand, const bool removeIsolationCut, const bool removeLifetimeRelatedCuts, const bool decayLengthSigniInverted )
{
...
{ 
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    std::vector<TreeDipseudoLeptonCandidate>::const_iterator cand = candidates->candidates_.begin();
