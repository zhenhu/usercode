...
#if (LeptonType == "Electrons" and clmode != 2):
#	drawExpected = 0
...
if (LeptonType.find("Electron") > -1):
        fr.SetTitle("CMS #sqrt{s}=7 TeV L="+lumiEle+" fb^{-1}")
	t1 = r.TLatex(xmin*2, ymax/3, "e^{+}e^{-}")
	t1.Draw()
elif (LeptonType.find("Muon") > -1):
	fr.SetTitle("CMS #sqrt{s}=7 TeV L="+lumiMu+" fb^{-1}")
...
for xmass in xmasses:
	...
	if len(flist) > 0:
		...
		for filename in flist:
			...
			if (lt < xmin or lt > xmax):
				sys.exit("ERROR PlotLimitsLifetimeExpected MH=%f: The x-axis range of your graph %f to %f is too small to include the lifetime point %f. Please increase it !" %(hmass, xmin, xmax, lt) )
			...
			print "PLOTTING ",filename," m=",m," tau=",lt," obs=",up," est=",est, " (1) ",est1p, "/", est1m, " (2) ",est2p, "/", est2m,"\n"
...
if ymin_seen < ymin or ymax_seen > ymax:
	sys.exit("ERROR PlotLimitsLifetimeExpected MH=%f: Either ymax is too small or ymin too large. TGraphErrors may plot error bands incorrectly unless you adjust them: %f --> %f %f --> %f" %(hmass, ymin, ymin_seen, ymax, ymax_seen))
