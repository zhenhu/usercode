...
def GetLimitBayesian(w,LeptonType,fileName):
	...
	# Note: this will only work properly for Electron41/Muon33
	if LeptonType.find("Electron") == 0:
		data = r.RooDataSet.read("WinterSelection/masses_data_electron.txt",r.RooArgList(mass)) 
	elif LeptonType.find("Muon") == 0:
		data = r.RooDataSet.read("WinterSelection/masses_data_muon.txt",r.RooArgList(mass)) 
	else:
